...
func (rc *RoleController) Update(ctx *gin.Context) {
	...
	err := ctx.ShouldBindJSON(&request)
	if err != nil {
		util.SendErrorResponse(ctx, http.StatusBadRequest, "error while binding")
		return
	}
...
// 		util.SendErrorResponse(ctx, http.StatusBadRequest, "error convert paramId")
// 		return
// 	}
